define(['jquery', 'underscore', 'lib/modules/gallery'], function($, _, gallery) {
    ...
    window.addEventListener('scroll', function(e) {
        if (document.body.scrollTop > 200 && !menuShrunk) {
            console.log('shrinking menu now');
            menuShrunk = true;
            title.style.marginTop = ".1em";
            titleLink.style.color = "#e2e2e2";
            navWrappper.style.height = "50px";
            for (i = 0; i < menuEls.length; i++) {
                menuEls[i].style.height = "50px";
                menuLinks[i].style.height = "50px";
                menuLinks[i].style.padding = ".9em";
            }
        }
        else if (document.body.scrollTop < 200 && menuShrunk) {
            console.log('restoring menu now');
            menuShrunk = false;
